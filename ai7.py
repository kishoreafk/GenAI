...
class Submission(models.Model):
    ...
    language = models.CharField(max_length=50, choices=[('python', 'Python'), ('java', 'Java'), ('cpp', 'C++'), ('javascript', 'JavaScript'), ('c', 'C')])
...
def execute_code(submission):
    try:
        ...
        container = client.containers.run(
            image_map.get(submission.language, "python:3.8"),
            command=f"python3 -c {submission.code}" if submission.language == 'python' else f"sh -c '{submission.code}'",
            mem_limit="128m",
            cpu_period=100000,
            cpu_quota=50000,
            detach=True,
